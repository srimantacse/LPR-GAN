...
from keras.layers import Input, concatenate, Conv2D, MaxPooling2D, UpSampling2D, Reshape, Dropout, Conv2DTranspose
...
class BMDDNet:
    def __init__(self, 
        ...
        self.d_model1        = self._define_patch_discriminator()
        self.d_model2        = self._define_image_discriminator()
    ...
    def run(self, model_dir, no_epoch=10, log_print=True):
        ...
        for epoch in range(no_epoch):
          for batch in range(self.iteartion_per_epoch):
            ...
            self.g_model.save(os.path.join(model_dir,  'BMDDNet_gen.h5'))
            self.d_model1.save(os.path.join(model_dir, 'BMDDNet_dis_patch.h5'))
            self.d_model2.save(os.path.join(model_dir, 'BMDDNet_dis_full.h5'))

          if (epoch % 10 == 0):
            print('Saving Model of iteration ' + str(epoch))
            self.g_model.save(os.path.join( model_dir,  'BBMDDNet_gen_' + str(epoch) + '.h5'))
    ...
    def _define_patch_discriminator(self, loss_in='binary_crossentropy'):
    ...
    def _define_image_discriminator(self, loss_in='binary_crossentropy'):
        image_shape = self.image_shape_dis
        init   = GlorotNormal()
        inputs = Input(shape=image_shape)
    ...
    def _define_gan(self, gen_loss_in, gen_loss_wghts):
      ...
      model  = Model(in_src, in_out, name="BMDDNet_GAN")
